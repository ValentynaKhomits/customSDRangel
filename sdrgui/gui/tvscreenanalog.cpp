...
static const char* fragmentShaderSource =
    ...
    "    float p2x = p1x + tlw;\n"
    "    float p4x = p3x + tlw;\n"
    "    vec4 color1 = texture(tex1, vec2(p1x, p1y));\n"
    "    vec4 color2 = texture(tex1, vec2(p2x, p1y));\n"
    "    vec4 color3 = texture(tex1, vec2(p3x, p3y));\n"
    "    vec4 color4 = texture(tex1, vec2(p4x, p3y));\n"
    "    vec3 color12 = mix(color1.rgb, color2.rgb, fract(txs1));\n"
    "    vec3 color34 = mix(color3.rgb, color4.rgb, fract(txs3));\n"
    "    vec3 finalColor = mix(color12, color34, fract(tys));\n"
